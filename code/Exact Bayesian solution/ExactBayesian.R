    ...
    # quicker than excel
    penalty <- c(1000000, 1000, 700.00000000, 330.07641174, 156.81303712,  74.49889703,  35.39301171,  16.81454797,   7.98827254,
    ...
    for (v in 1:length(training_size)){
      ...
      cv2 <- cv.indexing(k,nrow(dataset),percent)
...
##----------- Re-shuffling zero variance cases (incompatible with COR model) ------------------------------------------------------ 
          ...
          while (any(is.na(cov_mat))){
            if (i == 1000){trainset <- dataset[cv2[1+re[i],]==1, ] # re[i] = 0 at i=1
                           testset <- dataset[cv2[1+re[i],]==0, ]       
            }else{
...
#------Throwing out redundant predictors from x for both OLS and COR model fitting:      
     ...
     # deleting m-1 of the redundant predictors still gets rid of all redundancies
        trainset <- trainset[ ,-redundant[1:(m-1)]]
        testset <- testset[ ,-redundant[1:(m-1)]]
        Predictors <- ncol(trainset) - 1 # dependent
        cov_mat_new <- cor(trainset[ ,1:Predictors])
        ...
        # using solve:  X matrix without redundant predictors and the new test matrix without redundant predictors
        #predictions.LR[[i]] <- Linear.regression.cleaned(x, y, test, Predictors, labels) #
          
        ###--- ------------------  Regular Linear Regression: MAKE SURE IT USES manual computation as well!
        ...
        meanW <- vector('list',length(penalty)) 
          ...
          # cycling through all copies j = set j
              ...
              # stores all posterior weight matrices for each penalty parameter
              #meanW[[p]] <- w 
              
#         # These two order functions give different reuslts even when they are equivalent:
#           cue_order <- order(abs(cue_validities), decreasing = TRUE)       
#         # gives the same order as the matrix diagonal below! when ROUNDED. 
#         order(abs(round(cue_validities,2)), decreasing = TRUE)    
#            # the solution to get the equal  
#           temp <- diag(round(w, 2))
#           order(abs(temp), decreasing = TRUE) # gives the same as order(abs(cue_validities), decreasing = TRUE)
#           
        

          ##---------------------- PREDICTION: ---------------------------------------------      
           
          # if off-diagonals need to be even smaller, just increase penalty now. 
          # 185x15 times 15x15 = 185x15
          outcomes <- test %*% round(w,2)     ## ROUNDING IS THE SOLUTION, SHOULD ALSO TAKE CARE OF MCMC PROBLEM.
          # i checked even with large penalty, there are still precision erros if not rounded
          ...
          cv.run <- data.frame(data = paste(ALL_DATA[s]), v, percent_training, training_N = round(training_sample_size,0),  
                               ...
                               Predictors, N, k = k,  avg_reshuffle = re,  test_size = N - training_sample_size,
          ...
          save(results, file= paste("COR_", ALL_DATA[s],"_exactBayesian.RData", sep = ""))
          write.csv(results, file= paste("COR_", ALL_DATA[s],"_exactBayesian.csv", sep =""), row.names=TRUE)  
