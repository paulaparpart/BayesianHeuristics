...
## preamble 
    ...
    packages <- c('clusterGeneration', 'MASS', 'ggplot2', 'lars', 'psych', 'matrixcalc', 'abind', 'lattice', 'coda', 
                  'foreach', 'doParallel', 'tmvtnorm', 'gplots')
    lapply(packages, require, character.only = TRUE)
    
    #Set to the number of cores (Brad has 12)
    #registerDoMC(2)
    registerDoParallel(cores= 7)  ## 8 at work, 7 on mac 
    ...
    # oxidants run without first predictor
    #e <- 15
    # fuel world: run without n_licensed_drivers  
    #e <- 10
    
    # ozone
     e <- 20
     
    # new 0.0003 rule seems to have worked :)
    # agreement is now assessed without ties
    # truncated tallying knows cue directions in advance
    # halfridge model knows cue directions in advance
    # then we should get less-is-more effects with directed tallying
    ...
    # inside dopar
      ...
      if (e == 15){dataset <- dataset[ ,-5]}
      
      # oxygen with N = 115 did also not work
      if (e == 19){dataset <- dataset[ ,c(-7,-9,-10)]}
      
      # fuel world: run without n_licensed_drivers  
      #dataset <- dataset[ ,-7]
      
      # quicker than excel: another 1e07 for convergence
      # this is already 1/eta_2 so it is equal to parameter recovery penalty (model penalty)
      penalty <- c(1000000, 100000, 1000, 700.00000000, 330.07641174, 156.81303712,  74.49889703,  35.39301171,  16.81454797, 7.98827254,
                  3.79507663,  1.80296886, 0.85655628,  0.40693363,   0.19332644,   0.09184572,  0.03, 0.01, 0.001, 0.0001, 0.00001, 0)
      ...
      training_size <- round(0.9*N)
       
      #training_size <- c(10, 20, 115, 200)
...
#         training_size[3] <- 35 
#         training_size <- training_size[-4]
#       } else if (ALL_DATA[e] == "oxidants"){
#         training_size[4] <- 130
#        } else if (ALL_DATA[e] == "mortality"){
#         training_size[4] <- 180
#        } 
      ...
      chain_length[4] <- 100000

      for (v in 1:length(training_size)){
        
        # covariance convergence should be true for any training set size
        ...
        ties <- vector('numeric', k) 
        
        
        for (i in 1:k){  # test
          
          # reboot in case there was less predictors in last run
          cue_validities <- cue_validities_raw - 0.5
          ...
          if (any(cov_mat[lower.tri(cov_mat)]  > 0.99999999)){ # if there is at least 1 complete redundancy (TRUE) in the lower triangle,  
            ...
            Predictors <- ncol(trainset) - 1 # dependent
            cue_validities <- cue_validities[-redundant[1:(m-1)]] # update the length of this too, when there is a redundant predictor
          ...
          x <- as.matrix(trainset[ ,1:Predictors])   # Predictors needs to be updated in each loop 
          ...
          # learns the cue directions from training
          predictions.tallying[[i]] <- tallying.learning(trainset, test, Predictors)      
          ###--- ------------------  Truncated Tallying Heuristic 
          # knows the correct cue directions
          predictions.Trunctallying[[i]] <- truncated.tallying(cue_validities, trainset, test, Predictors)
          ...
          for (p in 1:length(penalty)){
            ...
            meanW[[p]] <- halfridge(cue_validities, Predictors,x, y, penalty[p], sigma_2, chain_length[v])
            ...
            # new epsilon rule
            epsilon <- 0.0003  #new epsilon cutoff that matches the e+04 changes necessary #0.00009
            
            outcomes[abs(outcomes) < epsilon] <- 0
            
            # made ties 0 above 
            outcomes <- sign(outcomes) # is vector anyway = predictions (not COR!)
            
          # count average real ties (not tallying based)
            ties[i] <- sum(outcomes == 0)
            # with "guessing" 
#             for (j in 1:length(outcomes)){  
#               if (outcomes[j]==0) outcomes[j] <- sign(rnorm(1))  
#             } 
            # will naturally lead to overlap of 50% with truncated tallying heuristic on those 0 cases
            ...
            #test the limit cases penalty 1 = 1e+06
...
#                       
        ...
        avg_percent_ties <- mean(ties)/length(test.labels)
      # This is not allowed: introduces bias
      # Get rid of NA runs: which columns of k columns can be used and index them
        ...
        S <- length(valid)
        if (S == 0){
          stop('all training sets produce NaN weights')
        }
        ...
        cv.run <- data.frame(guessing = 'no', data = paste(ALL_DATA[e]), v, percent_training, training_N = round(training_sample_size,0),  
                             ...
                             Tallying_Heuristic =  acc.tallying,
                             se_tallying_heuristic = se_tallying,    
                             
                             acc_LR = acc.LR,  se_acc_LR = se_acc_LR, 
                             TTB_Heuristic = acc.ttb, se_ttb_heuristic = se_ttb,
                             Agreement_TruncTallying_Halfridge = Agreement_Tallying_Halfridge,  
                             se_Tallying_Halfridge =  se_Tallying_Halfridge, 
                             avg_percent_ties =  avg_percent_ties,
      ...
      save(results, file= paste("HalfRidge_", ALL_DATA[e],"_mvrnorm_directed_90%_2.RData", sep = ""))
      write.csv(results, file= paste("HalfRidge_", ALL_DATA[e],"_mvrnorm_directed_90%_2.csv", sep =""), row.names=TRUE)  
