...
#   
    ...
    unitweights <- sign(round(cue_validities,2))      # make sure they are ROUNDED to 2 DECIMALS AS WELL.
...
ttb.predictions <- function(trainset, test,  Predictors)
{
  ...
  # trainset[ ,ncol(trainset)] still has y variable in it
    ...
    # the validities - 0.50 are automatically same sign as regression betas   
    #beta <- 2*(cue_validities_raw) - 1
    #order(abs(beta), decreasing = TRUE)
    
  # ROUND TO 2 DECIMALS to make EQUAL TO DIAGONAL ORDER
  
    cue_order <- order(abs(round(cue_validities,2)), decreasing = TRUE)
    ...
    for(r in 1:nrow(test)){
      ...
      while (test[r,cue_order[v]]== 0){            
        ...
        if (v == length(cue_order)){ 
          ttb_predict[r] <- 0 #sign(rnorm(1)) # if no cue discriminates, guess between A(+1) and B(-1) 
          break  
        }       
